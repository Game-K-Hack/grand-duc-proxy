    ...
    <div style="max-width:860px;margin:auto;display:flex;flex-direction:column;gap:20px">
      ...
      <div style="background:rgba(248,81,73,.1);border:1.5px solid var(--red);border-radius:8px;padding:18px 20px;display:flex;gap:14px;align-items:flex-start">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--red)" stroke-width="2" style="flex-shrink:0;margin-top:1px">
          ...
          <line x1="12" y1="9" x2="12" y2="13"/><circle cx="12" cy="17" r="1.2" stroke-width="0" fill="var(--red)"></circle>
        </svg>
        <div>
      ...
      <div class="card" style="padding:28px 24px">
        ...
        <svg v-else viewBox="0 0 700 200" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:700px;display:block;margin:auto">
          ...
          <g>
            ...
            <text x="45" y="190" text-anchor="middle" font-size="11" fill="var(--text-muted)">PCs</text>
          </g>
          ...
          <!-- Proxy -->
          <g>
            <rect x="220" y="65" width="110" height="70" rx="8"
              :fill="proxyFill"
              :stroke="proxyStroke"
              stroke-width="2"/>
      ...
      <div class="card" style="display:flex;align-items:center;justify-content:space-between;gap:20px;flex-wrap:wrap">
        ...
        <button
          class="btn"
          :style="active
            ? 'background:var(--green);color:#fff;border-color:var(--green);min-width:180px;font-size:15px;padding:12px 24px'
            : 'background:var(--red);color:#fff;border-color:var(--red);min-width:180px;font-size:15px;padding:12px 24px'"
          @click="toggle"
...
onMounted(async () => {
  const { data } = await killswitchApi.get()
...
async function toggle() {
  saving.value = true
  try {
    const { data } = await killswitchApi.set(!active.value)
    active.value = data.active
  } finally {
    saving.value = false
  }
